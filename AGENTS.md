...
The committed `skills/fortress-devtools-axi/SKILL.md` is generated by `pnpm run build:skill`; `pnpm test` fails if it drifts from the shared CLI guidance or documented SDK built-ins (`test/skill.test.ts`).
...
- Do not hand-edit generated files: `CHANGELOG.md` and `.release-please-manifest.json` (owned by release-please) or `skills/fortress-devtools-axi/SKILL.md` (owned by `build:skill`).
...
`src/skill.ts` renders the installable Agent Skill (`skills/fortress-devtools-axi/SKILL.md`) from the same shared guidance plus the SDK built-in command list, rewriting invocations to non-interactive `npx -y chrome-devtools-axi ...`.
